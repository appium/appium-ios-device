...
export class InstrumentService extends BaseServiceSocket {
  ...
  async _callChannel(sync, channelCode, selector, ...auxiliaries) {
    ...
    this._encoder.write({
      ...
      selector,
      auxiliaries:
        auxiliaries.length === 1 && auxiliaries[0] instanceof DTXMessageAuxBuffer ? auxiliaries[0] : auxiliaries,
      identifier,
    });
   ...
   */
  _handleData(data) {
    if (typeof data.selector === 'string' && data.selector.includes(CHANNEL_CANCELED)) {
      this.close();
    }
    if (data.conversationIndex === 1) {
      ...
      if (typeof selector === 'string' && this._callbacks.listenerCount(selector) > 0) {
