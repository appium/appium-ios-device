...
import * as plistlib from 'bplist-parser';
...
class Unarchive {
  ...
  unpackArchiveHeader() {
    ...
    const createPlistIssue = (/** @type {string} */ message) => {
      try {
        log.debug(`Source plist: ${util.truncateString(JSON.stringify(plist), {length: 250})}`);
      } catch {}
  ...
  decodeKey(obj, key) {
    const val = obj[key];
    return val?.UID == null ? val : this.decodeObject(val);
  }
...
class Archive {
  ...
  encodeDict(obj, archiveObj) {
    archiveObj['NS.keys'] = Object.keys(obj).map(this.archive.bind(this));
    archiveObj['NS.objects'] = Object.values(obj).map(this.archive.bind(this));
  ...
  encodeXCTestConfiguration(obj, archiveObj) {
    ...
    Object.entries(obj._data).forEach(
      ([key, value]) => (archiveObj[key] = key === FORMAT_VERSION ? this.archive(value) : this.encode(value)),
    );
  }
...
export function updateNSKeyedArchiveClass(name, subClass) {
  if (
    ...
    typeof subClass.prototype?.decodeArchive !== 'function' &&
    // @ts-ignore prototype always exists
    typeof subClass.prototype?.encodeArchive !== 'function'
  ) {
