 ...
 * Using this as a default decoder for unknown entities.
 */
...
class XCTestConfiguration extends BaseArchiveHandler {
  ...
  constructor(data) {
    ...
    if (!data.testBundleURL) {
      throw new Error('testBundleURL cannot be empty');
    }
    if (!data.sessionIdentifier) {
      throw new Error('sessionIdentifier cannot be empty');
    }
    ...
    if (!(data.testBundleURL instanceof NSURL)) {
      throw new Error('invalid testBundleURL');
    }
    ...
    if (!(data.sessionIdentifier instanceof NSUUID)) {
      throw new Error('invalid sessionIdentifier');
    }
...
class XCActivityRecord extends BaseArchiveHandler {
  ...
  decodeArchive(archive) {
    const ret = new XCActivityRecord();
    for (let key of XCActivityRecord._keys) {
...
class Archive {
  ...
  encodeTopLevel (obj, archiveObj) {
    if (obj instanceof Array) {
      ...
      if (objName in UNARCHIVE_CLASS_MAP) {
        ...
        if (obj.constructor.isNSObject) {
          archiveObj.$class = this.uidForArchiver(objName, 'NSObject');
        } else {
  ...
  encodeArray (objs, archiveObj) {
    archiveObj.$class = this.uidForArchiver('NSArray', 'NSObject');
    archiveObj['NS.objects'] = objs.map(this.archive.bind(this));
  }

  encodeSet (objs, archiveObj) {
    archiveObj.$class = this.uidForArchiver('NSSet', 'NSObject');
    archiveObj['NS.objects'] = objs.map(this.archive.bind(this));
  }
  ...
  encodeDict (obj, archiveObj) {
    ...
    archiveObj.$class = this.uidForArchiver('NSDictionary', 'NSObject');
  }
  ...
  encodeXCTestConfiguration(obj, archiveObj) {
    archiveObj.$class = this.uidForArchiver('XCTestConfiguration', 'NSObject');
    for (let key of Object.keys(obj._data)) {
      const value = obj._data[key];
      // This key must not be archived, so `encodeArchive` in the class is not used
      if (key !== 'formatVersion') {
        archiveObj[key] = this.encode(value);
      } else {
        archiveObj[key] = this.archive(value);
      }
    }
  }
