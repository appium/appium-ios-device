...
class XCTestConfiguration extends BaseArchiveHandler {
   ...
   *
   * @param {object} data
   * @param {string | NSURL} data.testBundleURL
   * @param {string | NSUUID} data.sessionIdentifier
   * @param {string?} data.targetApplicationBundleID
   */
  constructor(data) {
    super();
    ...
    if (!(data.testBundleURL instanceof NSURL)) {
      throw new TypeError('Expected testBundleURL to be a valid NSURL instance, got ${typeof testBundleURL} instead');
    }
    ...
    if (!(data.sessionIdentifier instanceof NSUUID)) {
      throw new TypeError('Expected sessionIdentifier to be a valid NSUUID instance, got ${typeof testBundleURL} instead');
    }
...
class XCActivityRecord extends BaseArchiveHandler {
  ...
  decodeArchive(archive) {
    const ret = {};
    for (const key of XCActivityRecord._keys) {
...
class Archive {
  ...
  encodeXCTestConfiguration(obj, archiveObj) {
    ...
    _.entries(obj._data).forEach((entry) => {
