...
class NSUUID extends BaseArchiveHandler {
   ...
   * @param {UUIDHelpers} helpers
   */
  constructor(data, helpers) {
    super();
    this._data = data;
    this.additionClasses = [NS_OBJECT];
    this.uuidStringify = helpers.stringify;
    this.uuidParse = helpers.parse;
  }

  decodeArchive(archive) {
    return this.uuidStringify(archive.decode('NS.uuidbytes'));
  }

  encodeArchive(obj, archive) {
    archive._archiveObj['NS.uuidbytes'] = Buffer.from(this.uuidParse(obj._data).buffer);
  }
}
...
class XCTestConfiguration extends BaseArchiveHandler {
  ...
  constructor(data, helpers) {
    ...
    if (typeof data.sessionIdentifier === 'string') {
      data.sessionIdentifier = new NSUUID(data.sessionIdentifier, helpers);
    }
...
/**
 * @typedef {Object} UUIDHelpers
 * @property {(s: Uint8Array, offset?: number) => string} stringify function to convert byte array to uuid string
 * @property {(s: string) => Uint8Array} parse function to convert uuid string to byte array
 */