...
export class WebInspectorService extends BaseServiceSocket {
  ...
  constructor(opts = {}) {
    ...
    if (
      typeof socketClient.setMaxSendFragment === 'function' &&
      typeof socketChunkSize === 'number' &&
      socketChunkSize > 0
    ) {
  ...
  /**
   * Initializes the data flow for iOS 11+.
   *
  ...
  /**
   * Initializes the data flow for iOS < 11, where data is separated into partial
  ...
  sendMessage(rpcObject) {
    ...
    if (this._verbose || lastError) {
      ...
      const messageString = util.jsonStringify(message, null);
      const rpcObjectString = util.jsonStringify(rpcObject, null);
      log.debug(messageString);
      if (messageString !== rpcObjectString) {
        log.debug('Original message:');
        log.debug(rpcObjectString);
      }
    }
...
export function cleanupRpcObject(obj) {
  ...
  if (!util.isPlainObject(obj) && !isArray) {
    return obj;
  }

  if (isArray) {
    return obj.filter((value) => value != null);
  }
