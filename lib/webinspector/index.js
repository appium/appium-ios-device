...
const getMaxFrameLength = () => {
  ...
  if (envValue) {
    ...
    log.warn(`Invalid APPIUM_IOS_MAX_FRAME_SIZE value: ${envValue}, using default: ${MAX_FRAME_SIZE}`);
  }
...
class WebInspectorService extends BaseServiceSocket {
  ...
  constructor (opts = {}) {
    ...
    if (_.isFunction(socketClient.setMaxSendFragment) && !_.isNil(socketChunkSize) && socketChunkSize && socketChunkSize > 0) {
    ...
    if (!isSimulator && !_.isNil(majorOsVersion) && majorOsVersion && majorOsVersion < PARTIAL_MESSAGE_SUPPORT_DEPRECATION_VERSION) {
