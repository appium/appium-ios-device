...
function getPlistString (plist) {
  // Buffers cannot be serialized in a readable way
  const bufferToJSON = Buffer.prototype.toJSON;
  delete Buffer.prototype.toJSON;
  try {
    return JSON.stringify(plist, (k, v) => Buffer.isBuffer(v) ? v.toString('utf8') : v, 2);
  } finally {
...
class WebInspectorService {
  ...
  sendMessage (rpcObject) {
    ...
    if (this._verbose) {
      ...
      log.debug(getPlistString(rpcObject));
    }
