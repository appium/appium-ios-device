...
async function startService (udid, serviceName, socket, connectCallback) {
  ...
  try {
    ...
    if (service.EnableServiceSSL) {
      return await connectPortSSL(udid, service.Port, socket, connectCallback);
    } else {
      return await connectPort(udid, service.Port, socket, connectCallback);
    }
  } finally {
    lockdown.close();
  }
