...
async function startService (udid, serviceName, socket, callback) {
  ...
  try {
    ...
    if (service.EnableServiceSSL) {
      return await connectPortSSL(udid, service.Port, socket, callback);
    } else {
      return await connectPort(udid, service.Port, socket, callback);
    }
  } finally {
    lockdown.close();
  }
