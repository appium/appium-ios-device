...
function getTextPlist (data) {
  ...
  if (data instanceof Buffer && PLIST_IDENTIFIER.BUFFER.compare(data, 0, PLIST_IDENTIFIER.BUFFER.length) === 0) {
...
function getBinaryPlist (data) {
  ...
  if (data instanceof Buffer && BPLIST_IDENTIFIER.BUFFER.compare(data, 0, BPLIST_IDENTIFIER.BUFFER.length) === 0) {
    return data;
  }
  return null;
}
