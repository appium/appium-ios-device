...
class LengthBasedSplitter extends Stream.Transform {
  ...
  constructor (littleEndian, maxFrameLength, lengthFieldOffset, lengthFieldLength) {
  ...
  _decode (data, pos) {
    ...
    let bytesToRead = (this.lengthFieldOffset + this.lengthFieldLength) - this._frameBufferIndex;
    let nBytesRead = this._readBytes(data, bufferMarker, this._frameBuffer, this._frameBufferIndex, bytesToRead);
    ...
    if (completeMessageLength > this.maxFrameLength) {
      throw new Error('The frame is bigger than expected');
    }
