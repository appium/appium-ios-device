...
import { toReadableSizeString } from '../../utilities';
...
const DEBUG_CONFIG = {
  ...
  largeFrameThreshold: parseInt(process.env.APPIUM_IOS_LARGE_FRAME_THRESHOLD || '10485760'), // 10MB
  ...
  _decode (data, pos) {
    ...
    if (DEBUG_CONFIG.enableFrameSizeLogging) {
      ...
      if (DEBUG_CONFIG.enableLargeFrameLogging && messageLength > DEBUG_CONFIG.largeFrameThreshold) {
        log.warn(`Large frame detected: ${toReadableSizeString(messageLength)}`);
        log.warn(`Service: ${this.serviceName}, Max allowed: ${toReadableSizeString(this.maxFrameLength)}`);
        
        // Log memory usage if monitoring is enabled
        if (DEBUG_CONFIG.enableMemoryMonitoring) {
          const memUsage = process.memoryUsage();
          log.warn(`Memory usage - RSS: ${toReadableSizeString(memUsage.rss)}, Heap: ${toReadableSizeString(memUsage.heapUsed)}`);
        }
      }
    
    if (messageLength > this.maxFrameLength) {
      // Enhanced error message with debugging info
      const errorMsg = `The frame is bigger than expected. Length: ${toReadableSizeString(messageLength)}, max: ${toReadableSizeString(this.maxFrameLength)}`;
      log.error(errorMsg);
