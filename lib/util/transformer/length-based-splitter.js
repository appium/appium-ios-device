...
export class LengthBasedSplitter extends Stream.Transform {
  constructor(opts) {
    ...
    const {readableStream, littleEndian, maxFrameLength, lengthFieldOffset, lengthFieldLength, lengthAdjustment} = opts;
  ...
  _decode(data, pos) {
    ...
    let nBytesRead =
      bytesToRead === 0
        ? 0
        : this._readBytes(data, bufferMarker, this._frameBuffer, this._frameBufferIndex, bytesToRead);
