...
export class LengthBasedSplitter extends Stream.Transform {
  ...
  _decode (data, pos) {
    ...
    if (messageLength > this.maxFrameLength) {
      this._handleFrameSizeError(messageLength);
    }
  ...
  /**
   * Handle frame size errors with enhanced debugging information
   * @param {number} messageLength - The size of the frame that exceeded the limit
   */
  _handleFrameSizeError(messageLength) {
    const errorMsg = `The frame is bigger than expected. Length: ${util.toReadableSizeString(messageLength)}, max: ${util.toReadableSizeString(this.maxFrameLength)}`;
    log.error(errorMsg);
