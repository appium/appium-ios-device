...
async function getDefaultSocket(opts = {}) {
  ...
  const useHostPort = !!(opts.socketHost || opts.socketPort || envOpts.socketHost || envOpts.socketPort);

  let socket;

  if (useHostPort) {
    // Connect using host/port if either is explicitly set
    socket = net.createConnection(socketPort, socketHost);
  } else if (await fs.exists(socketPath)) {
