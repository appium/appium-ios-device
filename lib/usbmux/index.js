...
async function getDefaultSocket(opts = {}) {
  ...
  const hasExplicitHostPort = !!(opts.socketHost || opts.socketPort || envOpts.socketHost || envOpts.socketPort);
  const isWinOrWsl = process.platform === 'win32' || (process.platform === 'linux' && /microsoft/i.test(os.release()));
