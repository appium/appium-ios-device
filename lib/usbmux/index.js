...
async function getDefaultSocket(opts = {}) {
  ...
  const envOpts = getSocketOptionsFromEnv(opts);

  const socketPath = opts.socketPath ?? envOpts.socketPath ?? defaults.socketPath;
  const socketHost = opts.socketHost || envOpts.socketHost || defaults.socketHost;
  const socketPort = opts.socketPort || envOpts.socketPort || defaults.socketPort;
  const timeout = opts.timeout ?? defaults.timeout;

  const useOptsPath = !!(opts.socketPath ?? envOpts.socketPath);
  const useOptsHostPort = !!(opts.socketHost || opts.socketPort || envOpts.socketHost || envOpts.socketPort);
  const usePath = useOptsPath || !useOptsHostPort;
 ...
 */
function getSocketOptionsFromEnv (opts = {}) {
