 ...
 * 
 * @param {?string} socketPath [/var/run/usbmuxd] The full path to the usbmuxd Unix socket
 * @param {?number} timeout [5000] The number of milliseconds to wait until
...
class Usbmux extends BaseServiceSocket {
  constructor (socketClient) {
    ...
    for (const evt in ['close', 'end']) {
      this._socketClient.prependOnceListener(evt, () => this._encoder.unpipe(this._socketClient));
    }
