 ...
 * @throws {Error} if connection timeout happened
...
export async function getDefaultSocket(opts = {}) {
  ...
  const hasOpts = Object.values(opts).some((v) => v != null);
  ...
  try {
    return await withTimeout(
      connectPromise,
      timeoutMs,
      new Error(`Failed to connect to usbmuxd within the ${timeoutMs}ms timeout`),
    );
  } catch (err) {
    if (!socket.destroyed) {
      // Avoid an unhandled 'error' event if ECONNRESET is emitted during destroy
      socket.once('error', () => {});
      socket.destroy();
    }
    throw err;
