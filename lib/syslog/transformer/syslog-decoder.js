...
const ESCAPE_CODE = '\\'.codePointAt(0);
const DASH = '-';
const DASH_SHIFT = 0x80;
const ARROW_UP = '^';
const ARROW_UP_SHIFT = 0x40;
const ESCAPE_TOKEN_LEN = 4;
const ESCAPED_CHAR_PATTERN = /\\M(-|\^)(.)/;


function toUtf8Byte (escapedChar, modifier) {
  switch (modifier) {
    case DASH:
      return escapedChar.codePointAt(0) + DASH_SHIFT;
    case ARROW_UP:
      return escapedChar.codePointAt(0) + ARROW_UP_SHIFT;
    default:
      return null;
  }
}
...
function unescape (logBuffer) {
  const parseUtf8Chars = (start) => {
    ...
    while (cursor < logBuffer.length) {
      ...
      const tokenMatch = ESCAPED_CHAR_PATTERN.exec(tokenStr);
      if (!tokenMatch) {
        break;
      }
  ...
  while (index < bytesView.length) {
    if (bytesView[index] === ESCAPE_CODE) {
