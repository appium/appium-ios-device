...
class Xctest {
    constructor(udid, xctestBundleId, targetBundleId = undefined) {
        this.udid = udid;
        this.running = false;
    ...
    async _launchAppRunner(majorVersion, sessionIdentifier) {
        ...
        const lookupResult = await installation.lookupApplications({ bundleIds: [this.xctestBundleId] });
        installation.close();
        ...
        const instrumentDaemon = await instrumentService.makeChannel(PROCESS_CONTROL);
        await instrumentService._callChannel(true, instrumentDaemon, 'processIdentifierForBundleIdentifier:', this.xctestBundleId);
        ...
        const pid = await instrumentService._callChannel(true, instrumentDaemon,
        ...
        await instrumentService._callChannel(true, instrumentDaemon, 'startObservingPid:', msg);
