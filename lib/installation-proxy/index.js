...
class InstallationProxyService {
   ...
   * @param {Array} returnAttributes the fields which should be filtered and returned to the client
   * @returns A map of the applications which the key is the bundleId
   */
  async listApplications (applicationType = 'User', returnAttributes = []) {
    const request = {
      Command: 'Browse',
      ClientOptions: {
    ...
    if (!_.isEmpty(returnAttributes)) {
      request.ClientOptions.ReturnAttributes = returnAttributes;
    }
